		...
		for (k=0;k<m;k++){
			...
			if (j > i){
				...
				if (r2OverSigma2 < CUTOFF_SQRD){
					...
					pressure += g*L*L*( dx*dx + dy*dy + dz*dz); // notice that that g = f/r, so this is f x r
...
int main(int argc, char **argv){
	...
	/*load the glass*/
	sprintf(snapFileName, "3dipl_glass_N%d_0.56_%.5d.dat", N, glass_serial);
	...
	sprintf(outFileName, "nvt_T%.4f_g%d.dat", T, glass_serial);
