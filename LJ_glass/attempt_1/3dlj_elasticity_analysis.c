...
/* ============================================================
 *  Shared-library entry points for Python ctypes
 *  Compile with:
 *    gcc -O2 -shared -fPIC -o libelasticity.so 3dlj_elasticity_analysis.c -lm
 * ============================================================ */

/* Load a snapshot and build bond list.  Returns 1 on success, 0 on failure. */
int lib_load_snapshot(const char *filename){
    allocate_memory();
    if (!readSnapShot((char *)filename))
        return 0;
    return 1;
}

void lib_free(){ free_everything(); }

int lib_contacts(){ return contacts; }

/* Evaluate u_eta_x_x_on_two_vectors with the SAME vec passed for both slots
 * (i.e. vec1 == vec2 == vec, the eigenvector psi_l).
 * Returns the scalar result. */
double lib_u_eta_xx(double *vec){
    return u_eta_x_x_on_two_vectors(vec, vec);
}

/* Evaluate tessianOnThreeVectors(v, u, u) where
 *   v = vna_eta (fixed, size 3N)
 *   u = psi_l   (eigenvector, size 3N)
 * Returns the scalar result. */
double lib_tessian_vna_psi_psi(double *vna, double *psi){
    return tessianOnThreeVectors(vna, psi, psi);
}

/* Compute BOTH terms for all modes in one C call.
 *   evecs  : (n_modes * 3N) row-major: evecs[l*3N + i] = psi_l[i]
 *   evals  : (n_modes,) eigenvalues omega^2
 *   vna    : (3N,)  vna_eta = H^{-1} xi_eta  (already sign-flipped by CG solver)
 *   n_modes: number of modes (after removing near-zero ones)
 *   out_t1 : pointer to scalar output for Term 1
 *   out_t2 : pointer to scalar output for Term 2
 */
void lib_compute_terms(double *evecs, double *evals, double *vna,
                       int n_modes, double *out_t1, double *out_t2){
    int l;
    double t1 = 0.0, t2 = 0.0;
    int stride = DIM * N;
    for (l = 0; l < n_modes; l++){
        double *psi = evecs + l * stride;
        double inv_w2 = 1.0 / evals[l];
        t1 += u_eta_x_x_on_two_vectors(psi, psi) * inv_w2;
        t2 += tessianOnThreeVectors(vna, psi, psi) * inv_w2;
    }
    *out_t1 = t1;
    *out_t2 = t2;
}
