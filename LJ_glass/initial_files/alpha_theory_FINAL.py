...
def load_elasticity_results(data_dir):
    raw = np.loadtxt(_p(data_dir, "3dlj_elasticity_N4000_s000.dat"), comments="#")
    out = {}
    for row in raw:
        g = int(row[0])
        out[g] = {"u_per_N": row[1], "pressure": row[2], "G": row[3], "K": row[4]}
    return out
...
def load_numerical_alpha(data_dir):
    ...
    with open(fname) as f:
        for line in f:
            ...
            parts = line.split()
            if len(parts) == 2:
                # simple "key value" line  (e.g. "dPdT 0.123", "K 0.456", "alpha 1.23e-5")
                k, v = parts
                meta[k] = float(v)
            elif len(parts) == 4:
                # per-glass line written by notebook: "glass_N  dPdT  K  alpha"
                label, dPdT_g, K_g, alpha_g = parts
                alpha_val = float(alpha_g)
                meta[label] = alpha_val          # store per-glass alpha keyed by label
                meta[f"{label}_dPdT"] = float(dPdT_g)
                meta[f"{label}_K"]    = float(K_g)
                per_glass_alphas.append(alpha_val)
            # lines with any other token count are silently skipped
    
    # If we don't have an "alpha" key but we have per-glass data, compute the mean
    if "alpha" not in meta and per_glass_alphas:
        meta["alpha"] = np.mean(per_glass_alphas)
        print(f"  [computed mean alpha from {len(per_glass_alphas)} glasses: {meta['alpha']:.6e}]")
    
    # Fallback: compute from dPdT and K if available
    if "alpha" not in meta and "dPdT" in meta and "K" in meta:
...
def make_plot(results, num_alpha, out_dir):
    ...
    if num_alpha and "alpha" in num_alpha:
...
def main():
    ...
    try:
        ...
        sys.exit("ERROR: 3dlj_elasticity_N4000_s000.dat not found in data-dir. "
                 "Run the C binary first (it writes that file plus hessian_g*.txt, "
                 "vna_eta_g*.txt, bonds_g*.txt, meta_g*.txt).")
    ...
    # Write alpha_theory_results.txt (renamed to match notebook expectations)
    out_path = os.path.join(args.out_dir, "alpha_theory_results.txt")
    with open(out_path, "w") as f:
        f.write("# Format: glass_N  alpha_harmonic  alpha_anharmonic  alpha_total  K\n")
        f.write("# (this format matches what thermal_expansion_lj_cleaned.ipynb expects)\n")
        for row in results:
            g = int(row[0])
            K = row[1]
            alpha_total = row[2]
            alpha_harm = row[3]
            alpha_anharm = row[4]
            # Write in format: glass_N  harmonic  anharmonic  total  K
            f.write(f"glass_{g}  {alpha_harm:+14.8g}  {alpha_anharm:+14.8g}  "
                    f"{alpha_total:+14.8g}  {K:14.8g}\n")
    print(f"\nWrote {out_path}")
    
    # Also write legacy format for backwards compatibility
    legacy_path = os.path.join(args.out_dir, "alpha_results.txt")
    ...
    print(f"Also wrote {legacy_path} (legacy format)")
    ...
    if num and "alpha" in num:
        ...
        print("  (alpha_numerical.txt found but missing 'alpha' key)")
        print(f"  Available keys: {list(num.keys())}")
    else:
