...
void write_bond_network(int g);   /* exports bond arrays for Python alpha_theory.py */
...
/* ── Write bond-network arrays so Python (alpha_theory.py) can reimplement
 *    u_eta_x_x_on_two_vectors and tessianOnThreeVectors without linking C.
 *
 *  bonds_g{g}.txt  — one row per contact:
 *      i   j   rij_x   rij_y   rij_z   first   second   third
 *  meta_g{g}.txt   — scalar globals:
 *      L  V  pressure  N  DIM  contacts
 *
 *  Together with hessian_g{g}.txt and vna_eta_g{g}.txt written by
 *  write_for_python(), this is everything alpha_theory.py needs.
 */
void write_bond_network(int g){
	int l;
	char nameBonds[64], nameMeta[64];
	FILE *fBonds, *fMeta;

	sprintf(nameBonds, "bonds_g%d.txt", g);
	sprintf(nameMeta,  "meta_g%d.txt",  g);

	fBonds = fopen(nameBonds, "w");
	fMeta  = fopen(nameMeta,  "w");

	/* scalar globals */
	fprintf(fMeta, "L %25.15g\n",        L);
	fprintf(fMeta, "V %25.15g\n",        V);
	fprintf(fMeta, "pressure %25.15g\n", pressure);
	fprintf(fMeta, "N %d\n",             N);
	fprintf(fMeta, "DIM %d\n",           DIM);
	fprintf(fMeta, "contacts %d\n",      contacts);
	fclose(fMeta);

	/* per-bond data */
	fprintf(fBonds, "# i  j  rij_x  rij_y  rij_z  first  second  third\n");
	for (l = 0; l < contacts; l++){
		int ii = lookupBond[2*l + I_INDEX];
		int jj = lookupBond[2*l + J_INDEX];
		fprintf(fBonds,
		        "%d\t%d\t%.15g\t%.15g\t%.15g\t%.15g\t%.15g\t%.15g\n",
		        ii, jj,
		        rij[DIM*l + X_COMP],
		        rij[DIM*l + Y_COMP],
		        rij[DIM*l + Z_COMP],
	...
	fclose(fBonds);
	printf("  wrote bonds_g%d.txt  meta_g%d.txt  (%d contacts)\n",
	       g, g, contacts);
}

/* alpha computed in Python (alpha_theory.py) — this stub kept so main compiles unchanged */
...
int main(int n, char **inputStrings){
	...
	for (g = 0; g < 10; g++){
		...
		if ( readSnapShot(snapFileName) ){
			...
			write_for_python(g);
			write_bond_network(g);   /* new: bond arrays for alpha_theory.py */
