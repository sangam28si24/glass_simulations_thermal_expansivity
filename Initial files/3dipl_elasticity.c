...
int main(int n,char **inputStrings){
	...
	if ( readSnapShot(snapFileName) ){
		printf("read %s\n\npressure:%.10g, u:%.10g ratio:%.10g\n\n",snapFileName, pressure, u/DOUBLE_N, typicalGrad/typicalContactForce);
		...
		printf("%d, u/N %.10g pressure %.10g, ",q, u/DOUBLE_N, pressure);
		printf("stress %.10g G=%.10g K=%.10g, ", stress, shear_modulus, bulk_modulus);
		printf("gna=%.10g, k_na=%.10g\n", g_na, k_na/( (double)(DIM*DIM) ) );
	}
	else
		printf("%s not about\n",snapFileName);
