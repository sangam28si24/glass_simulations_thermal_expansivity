/* 3dipl NVT code with Nose Hoover thermostat, updated May 13, 2025 */
#include <string.h>
#include <time.h>
#include <stdio.h>
#include <math.h>
...
//notice that here N = 4n^3 with n some integer. here n^3 = 512 = 8^3
#define	N							2048
...
void calculateForces(){
	...
	//set stuff to zero
	u = 0.0; stress = 0.0; typicalInteractionStrength=0.0;
		...
		for (k=0;k<m;k++){
			...
			if (j > i){
				...
				if (r2OverSigma2 < CUTOFF_SQRD){
					...
					g = (N_0*sigmaOverR12 - r2OverSigma2*(4.0*C_4 + r2OverSigma2*6.0*C_6) - 2.0*C_2)*invSigmaSqrd;
					temp = L*dx*g;
					fx[j] += temp;
					fx[i] -= temp;
					temp = L*dy*g;
					...
					pressure += g*L*L*( dx*dx + dy*dy ); // notice that that g = f/r, so this is f x r
...
void run_NH(double duration){
        int k,steps,between=128;
        FILE *outFile;
	
        outFile = fopen("NH_3dipl_test_T1.0.dat","wb");
        steps = (int)(duration/TIME_STEP);
...
int main(int n,char **inputStrings){
	...
	initialize_fcc_grid();
	T = 1.0; //make sure to set T!
	// run berendsen for a bit
	run_berendsen(25.0); //runs berendsen for 25 time units
	
	//now run with Nose Hoover thermostat
	run_NH(250.0); //runs for 250 time units
